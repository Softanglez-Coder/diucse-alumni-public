...
import { UserService, MembershipService, MembershipStatus, Membership } from "../../../services";
...
export class PortalMembership {
    ...
    protected isGuest(): boolean {
        ...
        return user?.roles?.includes('Guest') || user?.roles?.includes('guest') || false;
    }

    protected canApplyForMembership(): boolean {
        const membership = this.currentMembership();
        return this.isGuest() && (!membership || membership.status === MembershipStatus.Draft);
    }
    ...
    protected getMembershipStatusText(): string {
        ...
        switch (membership.status) {
            case MembershipStatus.Draft: return 'Draft';
            case MembershipStatus.Requested: return 'Under Review';
            case MembershipStatus.InProgress: return 'In Progress';
            case MembershipStatus.PaymentRequired: return 'Payment Required';
            case MembershipStatus.Approved: return 'Approved';
            case MembershipStatus.Rejected: return 'Rejected';
    ...
    protected getMembershipStatusBadgeClass(): string {
        ...
        switch (membership.status) {
            case MembershipStatus.Approved: return 'badge-success';
            case MembershipStatus.Requested:
            case MembershipStatus.InProgress: return 'badge-warning';
            case MembershipStatus.PaymentRequired: return 'badge-info';
            case MembershipStatus.Rejected: return 'badge-danger';
            case MembershipStatus.Draft: return 'badge-secondary';
    ...
    protected async submitMembershipRequest() {
        ...
        try {
            const result = await this.membershipService.requestMembership().toPromise();
            ...
            alert('Membership application submitted successfully! You will receive an email confirmation shortly.');

...
export class Members {
    ...
    protected members = toSignal(this.userService.getMembers(), { initialValue: [] });
    ...
    protected filteredMembers = computed(() => {
        ...
        return allMembers.filter((member: User) => {
            ...
            const matchesEmail = !emailFilter || member.email.toLowerCase().includes(emailFilter);
            const matchesPhone = !phoneFilter || member.phone.toLowerCase().includes(phoneFilter);
            const matchesCompany = !companyFilter || member.company.toLowerCase().includes(companyFilter);

...
export class MembershipService extends BaseService<Membership> {
    ...
    getMyMembership(): Observable<Membership | null> {
        return this.httpClient.get<Membership>(`${this.apiBaseUrl}/memberships/me`, {
    ...
    requestMembership(): Observable<Membership> {
        return this.httpClient.post<Membership>(`${this.apiBaseUrl}/memberships/request`, {}, {
            withCredentials: true
        }).pipe(
    ...
    getMembershipById(id: string): Observable<Membership> {
        return this.httpClient.get<Membership>(`${this.apiBaseUrl}/memberships/${id}`, {
            withCredentials: true
        }).pipe(
            map(response => this.transformResponse(response))
        );
